...
public abstract class MonthView extends View {
  ...
  //  private float rightSpace = 35;
//  private float rightSpace = 40;
  ...
  public MonthView(Context context, AttributeSet attr, DatePickerController controller) {
    ...
    mStringBuilder = new StringBuilder(10);
    ...
    mRowHeight = (res.getDimensionPixelOffset(R.dimen.mdtp_date_picker_view_animator_height) - getMonthHeaderSize()) / MAX_NUM_ROWS;
...
//    Display display = ((View)context).getWindowManager().getDefaultDisplay().getMetrics(displayMetrics);
...
//    Log.i("MWIDTH___AA", "________mWidth:" + mWidth);
...
//    int dayWidthHalf = 15;
    ...
    float firstX = (2.16f * (mNumDays - 1) + 1) * dayWidthHalf + rightSpace;
    for (int i = 0; i < mNumDays; i++) {
      int calendarDay = (i + mWeekStart) % mNumDays;
//      int x = (int) (firstX - (2 * i + 1) * dayWidthHalf + mEdgePadding);
      int x = (int) (firstX - (2 * i + 1) * dayWidthHalf + mEdgePadding) /*+ 60*/;
...
//      Log.i("MWIDTH___AA", "weekString:" + weekString);


      Log.i("MWIDTH___AA",
        "______Labels firstX:" + firstX +
          "______Labels (2 * i + 1):" + (2 * i + 1) +
          "______Labels dayWidthHalf:" + dayWidthHalf +
          "______Labels mEdgePadding:" + mEdgePadding +
          "______Labels =====:" + (firstX - (2 * i + 1) * dayWidthHalf + mEdgePadding)
      );
...
*/
  ...
  protected void drawMonthNums(Canvas canvas) {
    ...
    Log.i("UUU___CC_M", "mWidth:" + mWidth + "  (mWidth - mEdgePadding * 2):" + (mWidth - mEdgePadding * 2) + " mNumDays:" + mNumDays + "  (mNumDays * 2.0f):" + (mNumDays * 2.0f));
    int j = findDayOffset();
//    float firstX = (2 * (mNumDays - 1) + 1) * dayWidthHalf + rightSpace;
    float firstX = (2.16f * (mNumDays - 1) + 1) * dayWidthHalf + rightSpace;
    for (int dayNumber = 1; dayNumber <= mNumCells; dayNumber++) {
      final int x = (int) (firstX - (((2 * j + 1) * dayWidthHalf + mEdgePadding))) /*+ 60*/;


      Log.i("MWIDTH___AA",
        "______MonthNums firstX:" + firstX +
          "______MonthNums (2 * j + 1):" + (2 * j + 1) +
          "______MonthNums dayWidthHalf:" + dayWidthHalf +
          "______MonthNums mEdgePadding:" + mEdgePadding +
          "______MonthNums =====:" + (firstX - (((2 * j + 1) * dayWidthHalf + mEdgePadding)))
      );
